...
public:
    ...
    SIPP()  {}
    ~SIPP() {}
    Path find_path(const Agent& agent, const Map &map, std::list<Constraint> cons, Heuristic &h_values);

private:
    std::vector<Path> find_partial_path(std::vector<Node> starts,
                                        std::vector<Node> goals,
                                        const Map &       map,
                                        const Agent &     agent,
                                        Heuristic &       h_values,
                                        double            max_f = CN_INFINITY);

    Path add_part(Path result, Path part);

    void find_successors(Node              curNode,
                         const Map &       map,
                         const Agent &     agent,
                         std::list<Node> & succs,
                         Heuristic &       h_values,
