...
public:
    ...
    bool validate_constraints(std::list<Constraint> constraints,
                              opt_agent_ref_t       agent);

    bool check_positive_constraints(std::list<Constraint> constraints, Constraint constraint);

    std::optional<Conflict> check_paths(const sPath &pathA, const sPath &pathB);
    ...
    Constraint get_constraint(opt_agent_ref_t agent, Move move1, Move move2);

    Constraint get_wait_constraint(opt_agent_ref_t agent, Move move1, Move move2);
    ...
    double get_cost(CBS_Node node, opt_agent_ref_t agent);
