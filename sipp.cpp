...
{
    if (!this->agent) {
        throw std::runtime_error("SIPP::find_successors called without agent");
    }
...
Path SIPP::find_path(Agent _agent, const Map &map, std::list<Constraint> cons, Heuristic &h_values)
{
    this->clear();
    this->agent = _agent;
